...
def build_notifiers(cfg: Settings, cities: CityDirectory | None = None) -> list[Notifier]:
    """Fan-out targets from config; falls back to console so deals are never silent."""
    notifiers: list[Notifier] = []
    if cfg.telegram_bot_token and cfg.telegram_chat_id:
        notifiers.append(
            TelegramNotifier(cfg.telegram_bot_token, cfg.telegram_chat_id, cities=cities)
        )
